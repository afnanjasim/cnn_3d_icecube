...
def get_data(filename_list,file_type):
    ''' file_type="sig" or "bg" '''
    ...
    for fn in filename_list[1:]:
        xs,ys,wts = make_dataset(fn, file_type)
        x = np.vstack((x,xs))
        y = np.concatenate((y,ys))
        wt = np.concatenate((wt,wts))
...
#     for file_list,sig_or_bg in zip([sig_list,bg_list],['sig']):

        print('Type: ',sig_or_bg)
#         block_size=min(100,len(file_list))
        ...
        for i in np.arange(num_blocks):
            ...
            inx,inpy,wts = get_data(f_list,sig_or_bg)
